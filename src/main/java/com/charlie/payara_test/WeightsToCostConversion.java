		...
		for(int i = 0; i < weights.length; i++) {
			...
			for(RuleNames item : RuleNames.values()) {
				...
				case "FREE_UNDER_7KG_OVERWEIGHT":
					if (singles.get("FREE_UNDER_7KG_OVERWEIGHT") == USED_ONCE)
						if (weights[i] > item.getOver() && weights[i] < item.getUnder()) {
							singles.put("FREE_UNDER_7KG_OVERWEIGHT", 0);
							contin = true;
				...
				case "FREE_UNDER_25KG_OVERWEIGHT":
					if (singles.get("FREE_UNDER_25KG_OVERWEIGHT") == USED_ONCE)
						if (weights[i] > item.getOver() && weights[i] < item.getUnder()) {
							singles.put("FREE_UNDER_25KG_OVERWEIGHT", 0);
							contin = true;
						} else if (weights[i] > item.getUnder()) {
							sum += RuleNames.valueOf("OVERWEIGHT").getPrice() * (weights[i] - item.getUnder());
							singles.put("FREE_UNDER_25KG_OVERWEIGHT", 0);
					...
					break;
				case "FEE_UNDER_7KG_OVERWEIGHT":
					if (singles.get("FEE_UNDER_7KG_OVERWEIGHT") == USED_MANY)
						if (weights[i] > item.getOver() && weights[i] < item.getUnder()) {
